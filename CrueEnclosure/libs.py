...
def excel_is_valid(file_excel_path):
    ...
    if (file_excel_path.endswith(".xlsx") and os.path.isfile(file_excel_path)): #mira si termina en XLSX y si existe el archivo
        excel_is_valid=True
    else:
...
def append_df_to_excel(df, excel_path,sheet_name):
    if(os.path.isfile(excel_path)): #Mira si el archivo excel existe
