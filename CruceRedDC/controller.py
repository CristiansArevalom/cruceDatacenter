...
class Controller:
    ...
    def main (self):
        self.view.main()
    def on_botonSeleccionarArchitoTorreCorp_click(self):
        #Extraer ruta dede la vista
        view_file_tower_path=self.view.read_user_path_files()
        view_file_tower_path+=""
        self.view.set_user_file_tower_path(view_file_tower_path)
        #Enviando ruta al model
        self.model.set_tower_inventory_path_router(view_file_tower_path)
        print(f"Valor en model:{self.model.get_tower_inventory_path_router()}")
        print(f"Valor en view:{self.view.get_user_file_tower_path()}")
    ...
    def on_botonEjecutar_click(self):
        torre_inventory_path=self.model.get_tower_inventory_path_router()
